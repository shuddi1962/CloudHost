...
export default function AuthSettingsPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const [provRes, infoRes] = await Promise.all([
        fetch("http://localhost:3001/api/auth-providers/organization/00000000-0000-0000-0000-000000000000", { headers }),
        fetch("http://localhost:3001/api/auth-providers/providers-info", { headers }),
      ]);
  ...
  const configureProvider = async (e: React.FormEvent) => {
    ...
    const token = localStorage.getItem("token");
    await fetch("http://localhost:3001/api/auth-providers/configure", {
      method: "POST",
  ...
  const toggleProvider = async (id: string) => {
    const token = localStorage.getItem("token");
    await fetch(`http://localhost:3001/api/auth-providers/${id}/toggle`, {
      method: "POST",
