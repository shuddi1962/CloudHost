...
export default function PreviewDeploymentsPage() {
  ...
  const fetchPreviews = async () => {
    ...
    const res = await fetch(`http://localhost:3001/api/preview-deployments/deployment/${deploymentId}`, {
  ...
  const createPreview = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = localStorage.getItem("token");
    const res = await fetch("http://localhost:3001/api/preview-deployments", {
      method: "POST",
  ...
  const rebuild = async (id: string) => {
    const token = localStorage.getItem("token");
    await fetch(`http://localhost:3001/api/preview-deployments/${id}/rebuild`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    fetchPreviews();
  ...
  const deletePreview = async (id: string) => {
    ...
    const token = localStorage.getItem("token");
    await fetch(`http://localhost:3001/api/preview-deployments/${id}`, {
      method: "DELETE",
      headers: { Authorization: `Bearer ${token}` },
    });
    fetchPreviews();
