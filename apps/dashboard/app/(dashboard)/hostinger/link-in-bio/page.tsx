...
export default function LinkInBioPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3001/api/hostinger-services/link-in-bio", {
  ...
  const create = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = localStorage.getItem("token");
    const res = await fetch("http://localhost:3001/api/hostinger-services/link-in-bio", {
      method: "POST",
  ...
  const publish = async (id: string) => {
    setPublishing(id);
    const token = localStorage.getItem("token");
    const res = await fetch(`http://localhost:3001/api/hostinger-services/link-in-bio/${id}/publish`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.ok) {
