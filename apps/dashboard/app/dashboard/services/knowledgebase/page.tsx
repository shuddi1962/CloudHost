...
export default function KnowledgebasePage() {
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/marketing-suite/knowledgebase?${params}`, { headers });
  ...
  const viewArticle = async (article: Article) => {
    setSelected(null);
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/marketing-suite/knowledgebase/${article.slug}`, { headers });
      const data = await res.json();
  ...
  const vote = async (helpful: boolean) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/marketing-suite/knowledgebase/${selected.id}/vote`, {
        method: "POST", headers,
