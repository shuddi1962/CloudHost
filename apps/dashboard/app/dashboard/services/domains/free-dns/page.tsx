...
export default function FreeDnsPage() {
  ...
  const fetchDns = async () => {
    try {
      const res = await fetch("http://localhost:3001/api/domain-services/free-dns", { headers });
  ...
  const addDomain = async (e: React.FormEvent) => {
    e.preventDefault();
    const res = await fetch("http://localhost:3001/api/domain-services/free-dns", {
      method: "POST", headers,
  ...
  const deleteDns = async (id: string) => {
    const res = await fetch(`http://localhost:3001/api/domain-services/free-dns/${id}`, {
