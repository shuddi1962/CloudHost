...
export default function FTPAccountsPage() {
  ...
  const fetchAccounts = async () => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hosting/account/00000000-0000-0000-0000-000000000000/ftp`, {
  ...
  const createAccount = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = localStorage.getItem("token");
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hosting/ftp`, {
      method: "POST",
  ...
  const resetPassword = async (id: string) => {
    const token = localStorage.getItem("token");
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hosting/ftp/${id}/reset-password`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    const data = await res.json();
  ...
  const deleteAccount = async (id: string) => {
    ...
    const token = localStorage.getItem("token");
    await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/hosting/ftp/${id}`, { method: "DELETE", headers: { Authorization: `Bearer ${token}` } });
    fetchAccounts();
  };
