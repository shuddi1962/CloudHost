...
export default function HorizonsPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3001/api/hostinger-services/horizons", {
  ...
  const create = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = localStorage.getItem("token");
    const res = await fetch("http://localhost:3001/api/hostinger-services/horizons", {
      method: "POST",
  ...
  const generate = async (id: string) => {
    ...
    setGenerating(id);
    const token = localStorage.getItem("token");
    const res = await fetch(`http://localhost:3001/api/hostinger-services/horizons/${id}/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
  ...
  const publish = async (id: string) => {
    setPublishing(id);
    const token = localStorage.getItem("token");
    const res = await fetch(`http://localhost:3001/api/hostinger-services/horizons/${id}/publish`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.ok) {
