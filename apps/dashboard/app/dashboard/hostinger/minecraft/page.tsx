...
export default function MinecraftPage() {
  ...
  useEffect(() => {
    ...
    fetch("http://localhost:3001/api/hostinger-services/minecraft", {
  ...
  const create = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = localStorage.getItem("token");
    const res = await fetch("http://localhost:3001/api/hostinger-services/minecraft", {
      method: "POST",
  ...
  const action = async (id: string, action: string) => {
    const token = localStorage.getItem("token");
    const res = await fetch(`http://localhost:3001/api/hostinger-services/minecraft/${id}/${action}`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
